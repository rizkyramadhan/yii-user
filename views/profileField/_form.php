<div class="form">
    ...
    <div class="row varname">
        ...
        <p class="hint"><?php echo UserModule::t("Variable Name (lowercase letters and digits)."); ?></p>
    </div>
    ...
    <div class="row title">
        ...
        <p class="hint"><?php echo UserModule::t('Variable Title (maximum 255 Characters).'); ?></p>
    </div>

    <div class="row field_type">
        <?php echo CHtml::activeLabelEx($model, 'field_type'); ?>
        <?php echo (($model->id) ? $model->field_type . "<br/><br/>" : CHtml::activeDropDownList($model, 'field_type', ProfileField::itemAlias('field_type'), array('id' => 'field_type'))); ?>
    ...
    <script type="text/javascript">
        $(function() {
            if ($("#ProfileField_visible").val() == "<?php echo ProfileField::VISIBLE_FOR_CERTAIN_ROLE; ?>") {
                $(".visible_for_role").show();
            } else {
                $(".visible_for_role").hide();
            }
            
            $("#ProfileField_visible").change(function() {
                if ($("#ProfileField_visible").val() == "<?php echo ProfileField::VISIBLE_FOR_CERTAIN_ROLE; ?>") {
                    $(".visible_for_role").show();
                    $("#ProfileField_visible_for_role").focus();
                } else {
                    $(".visible_for_role").hide();
                }
            });
